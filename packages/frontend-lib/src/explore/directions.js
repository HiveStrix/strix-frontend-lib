...
export const DIRECTIONS = [
  ...
  },
  {
    ...
    form: 'Plantilla nueva, no una variante. Toma tres cosas de tres sitios y las reescribe juntas. De Prisma, la FORMA: radio de 12 y 22, superficies opacas, sombra real, pasteles de estado. La TABLA quería la escalera de densidad de tres escalones de Cristal —cabecera sólida porque es lo que no se mueve, fila bajo el cursor tenue, fila en reposo transparente—, que es lo único que mantiene legible una tabla de ocho columnas, pero Cristal no se puede copiar tal cual sobre blanco: su superficie es translúcida (blanco al 56 %) y esa escalera está hecha de transparencia sobre el campo teñido que Cristal pone detrás, así que sobre el blanco liso de Nácar los mismos valores resuelven a blanco puro y la escalera desaparece. Por eso la tabla se toma, valor por valor y no regla por regla, de Z · Prisma pastel liso: la misma escalera de tres escalones, pero opaca, así que se reproduce exacta sin depender de que haya un campo detrás. De Halo claro, la BARRA: no se separa con una línea sino con la luz que deja caer, y acá va a un tercio de la fuerza de T, que sobre blanco es lo que hace la diferencia entre una barra que flota y una barra que gotea. El resto es la ley de la librería: el cromo no lleva color. Etiquetas en tinta, líneas en gris con una traza de marca para que no se lean verdosas al lado del acento, fondo blanco liso. El acento y el estado son el único color de la pantalla, y por eso los dos se ven sin explicarse.',
    mezcla: 'forma de Prisma + tabla de Prisma pastel liso, por valor (Cristal era la primera opción y resulta irreproducible sobre blanco) + barra de Halo claro',
    finalista: true
  }
];
